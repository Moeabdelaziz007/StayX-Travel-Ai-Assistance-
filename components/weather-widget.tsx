...
export function WeatherWidget({ location: initialLocation = 'Paris, France' }: { location?: string }) {
  ...
  const getIcon = (condition: string) => {
    switch (condition?.toLowerCase()) {
      case 'sunny': return <Sun className="h-12 w-12 text-yellow-400" />;
      case 'rainy': return <CloudRain className="h-12 w-12 text-blue-400" />;
      case 'windy': return <Wind className="h-12 w-12 text-zinc-400" />;
      default: return <Cloud className="h-12 w-12 text-zinc-300" />;
    }
  };
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <div className="text-lg font-medium text-zinc-200">{weather?.condition}</div>
                </div>
